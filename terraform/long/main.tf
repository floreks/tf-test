...
# Create a random string for each item in the 'strings' set
...
# Print each random string and sleep for 5 seconds to simulate work
...
# Output the random strings as a sensitive value
output "sensitive" {
  value = [ for i, s in random_string.random : s.result ]
  sensitive =  true
}

# Output the random strings as a standard value
output "standard" {
  value = [ for i, s in random_string.random : s.result ]
  sensitive =  false
}